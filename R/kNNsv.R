#' kNN with grid search variable selection
...
# cl_train<- cl_test<- factor(c(rep("s",25), rep("c",25), rep("v",25)))
# k<- 5
# kNNvs(train_x,test_x,cl_train,cl_test,k)

# kNN drop-one variable selection
#
# kNNvsd<- function(train_x,test_x,cl_train,cl_test,k)    #drop-one selection
# {
#   nnew     <- nrow(test_x)
#   yhat.knn <- numeric(nnew)
#   n_x<- ncol(test_x)
#   pcc_te_max<- rep(0,n_x)
#   bag<- c(1:n_x)                              # the number of variable in the test data, since its add one so there will be n(test_x) round comparsion
#   n_max<- 0                                   # the variable win around the peer model
#   a <- 0                               # save the variable information,the wining model's variabls
#   outcome_list<- as.list(NA)                  # the wining model includs varibals
#   for (h in 1:n_x) {
...
#         xn <- matrix(rep(t(test_xnew[i,]), times=nrow(train_xnew)), byrow=T, ncol=ncol(train_xnew))
#         #dist.x <- sqrt(rowSums((as.matrix(xn)-as.matrix(train_xnew))^2))
#         dist.x <- sqrt(rowSums((xn-train_xnew)^2))
#         s.dist <- sort(dist.x, index=T)
#         neighbors <- s.dist$ix[1:k]
#         w.x <- 1/(1+dist.x[neighbors])
#         w <- w.x/sum(w.x)
...
#   }
#
#   return(list(acc=pcc_te_max,outcome=outcome_list, bestacc = max(pcc_te_max)[1], bestset = (outcome_list[pcc_te_max == max(pcc_te_max)][1]) ) )
#
# }
